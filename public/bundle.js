...
const VimeoInteractiveFiction = __webpack_require__(34);

class Demo extends React.Component {
    render() {
        return (
            React.createElement(VimeoInteractiveFiction, {settings: [
                {
...
const React = __webpack_require__(2);
const ReactDOM = __webpack_require__(10);
const Vimeo = __webpack_require__(35);
const SceneMarker = __webpack_require__(55);
...
class VimeoInteractiveFiction extends React.Component {
    ...
    render() {
        ...
        const markers = this.props.settings[this.state.curIndex].markers.map((item, index) => {
            return React.createElement(SceneMarker, {triggerTime: item.triggerTime, position: item.position, targetIndex: item.targetIndex, key: item.pos})
        });
        ...
        return (
            React.createElement("a-scene", {id: "vimeoscene", key: this.state.curIndex}, 

                /*Dynamic Scene Adjustments*/
                /*We use the url as a UID for React's 'key' property, forcing a full remount when the key changes*/
                React.createElement(Vimeo, {url: url, position: "0 3 -15"}, 
                    markers
                ), 


                /* Boilerplate Scene Resources, a camera and cursor*/
                React.createElement("a-camera", {position: "0 0 0"}, 
                    React.createElement("a-cursor", {visible: "true"})
                )
            )
        );
...
class Vimeo extends React.Component {
  ...
  render() {
    ...
    var video_assets =
      React.createElement("a-assets", {timeout: "1"}, 
        React.createElement("video", {id: "vimeo", style: {display: "none"}, autoPlay: true, loop: true, crossOrigin: "anonymous", playsInline: true, "webkit-playsinline": "true", muted: this.props.muted}, 
          this.state.videos.map(function(video, i){
            if (video != null) {
              if (video['width'] != null) {
                aspect_ratio = parseInt(video['height']) / parseInt(video['width']);
                return React.createElement("source", {key: i, type: video['type'], src: video['link'], width: video['width'], height: video['height']})
              }
              else {
                return React.createElement("source", {key: i, type: video['type'], src: video['link']})
              }}
          })
        )
      );
    ...
    if (this.state.spatial) {
      ...
      if (this.state.stereo_format != 'mono') {
        video_display = [
          React.createElement("a-entity", {geometry: "primitive: sphere;" + " " +
            "radius: 100;" + " " +
            "segmentsWidth: 64;" + " " +
            "segmentsHeight: 64;", 
    ...
    // Regular video
      ...
      video_display = React.createElement("a-video", {src: "#vimeo", 
        width: width, 
        height: width * aspect_ratio, 
        position: this.props.position, 
        rotation: this.props.rotation, 
        scale: this.props.scale, 
        "play-on-window-click": true, "play-on-vrdisplayactivate-or-enter-vr": true});
    }
...
class SceneMarker extends React.Component {
    ...
    render() {
        if (this.state.isActive) {
            ...
            return (
                    React.createElement("a-box", {"data-targetindex": targetIndexString, "cursor-listener": true, position: this.props.position, scale: "2 2 2", color: "#00FF00"})
            )
        } else {
            return null;
        }
